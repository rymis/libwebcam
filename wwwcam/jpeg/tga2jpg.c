...
static void image_compare(struct image_compare_results *results, int width, int height, const uint8 *pComp_image, int comp_image_comps, const uint8 *pUncomp_image_data, int uncomp_comps, int luma_only)
{
  double hist[256];
...
static int exhausive_compression_test(const char *pSrc_filename)
{
  ...
  struct image_compare_results prev_results;

  uint quality_factor, subsampling, optimize_huffman_tables;

  memset(&prev_results, 0, sizeof(prev_results));

  for (quality_factor = 1; quality_factor <= 100; quality_factor++)
  {
    for (subsampling = 0; subsampling <= JPGE_H2V2; subsampling++)
    {
      for (optimize_huffman_tables = 0; optimize_huffman_tables <= 1; optimize_huffman_tables++)
      {
        // Fill in the compression parameter structure.
        struct jpeg_params params;
	jpeg_params_init(&params);
        params.m_quality = quality_factor;
        params.m_subsampling = (subsampling);
        params.m_two_pass_flag = (optimize_huffman_tables != 0);

        int comp_size = orig_buf_size;
        if (!compress_image_to_jpeg_file_in_memory(pBuf, &comp_size, width, height, req_comps, pImage_data, &params))
        {
          status = EXIT_FAILURE;
          goto failure;
        }
        ...
        struct image_compare_results results;
        image_compare(&results, width, height, pImage_data, req_comps, pUncomp_image_data, uncomp_req_comps, (params.m_subsampling == JPGE_Y_ONLY) || (actual_comps == 1) || (uncomp_actual_comps == 1));
...
int main(int arg_c, char* ppArgs[])
{
  ...
  // Parse command line.
  int run_exhausive_test = FALSE;
  int test_memory_compression = FALSE;
  int optimize_huffman_tables = FALSE;
  int subsampling = -1;
  ...
  if (run_exhausive_test)
  {
    ...
    return exhausive_compression_test(pSrc_filename);
  }
  ...
  jpeg_params_init(&params);
  params.m_quality = quality_factor;
  params.m_subsampling = (subsampling < 0) ? ((actual_comps == 1) ? JPGE_Y_ONLY : JPGE_H2V2) : subsampling;
  ...
  double total_comp_time = timer_get_elapsed_secs(&tm) * 1000.0;
  ...
  double total_uncomp_time = timer_get_elapsed_secs(&tm) * 1000.0;
