...
int http_root(mihl_cnx_t * cnx, char const *tag, char const *host, void *param)
{
	...
	mihl_add(cnx,
		 "<div style='text-align: center; font-weight: bold;'><big>Test AJAX");
	mihl_add(cnx, "(prototype.js)<br>");
	mihl_add(cnx, "</big></div>");
	mihl_add(cnx, "<br>");
	mihl_add(cnx,
		 "<div style='text-align: center;'><img style='width: 70px; height: 72px;'");
	mihl_add(cnx, " alt='' src='/image.jpg'><br>");
	mihl_add(cnx, "</div>");
	mihl_add(cnx, "<br>");
	mihl_add(cnx, "<table");
	mihl_add(cnx,
		 " style='text-align: left; background-color: rgb(208, 208, 208); margin-left: auto; margin-right: auto;'");
	mihl_add(cnx, " border='1' cellpadding='2' cellspacing='2'>");
	mihl_add(cnx, "  <tbody>");
	mihl_add(cnx, "    <tr>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;'>Refreshed each second<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;' id='field1'>XXXX<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx, "    </tr>");
	mihl_add(cnx, "    <tr>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;'>Refreshed each 5 seconds<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;' id='field2'>XXXX<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx, "    </tr>");
	mihl_add(cnx, "    <tr>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;'>Refreshed each 30 seconds<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx,
		 "      <td style='vertical-align: top;' id='field3'>XXXX<br>");
	mihl_add(cnx, "      </td>");
	mihl_add(cnx, "    </tr>");
	mihl_add(cnx, "  </tbody>");
	mihl_add(cnx, "</table>");
	...
	mihl_add(cnx,
		 "  var myAjax = new Ajax.Request( url, { method: 'get', onComplete: showData1 });");
	mihl_add(cnx, "};");

	mihl_add(cnx, "function timerFunction5( ) {");
	mihl_add(cnx, "  var url = 'http://%s/data2';", host);
	mihl_add(cnx,
		 "  var myAjax = new Ajax.Request( url, { method: 'get', onComplete: showData2 });");
	mihl_add(cnx, "};");

	mihl_add(cnx, "function timerFunction30( ) {");
	mihl_add(cnx, "  var url = 'http://%s/data3';", host);
	mihl_add(cnx,
		 "  var myAjax = new Ajax.Request( url, { method: 'get', onComplete: showData3 });");
	mihl_add(cnx, "};");

	mihl_add(cnx, "function showData1( originalRequest ) {");
	mihl_add(cnx,
		 "  document.getElementById('field1').childNodes[0].nodeValue = originalRequest.responseText;");
//  mihl_add( cnx, "  field1.innerText = originalRequest.responseText;" );
	mihl_add(cnx, "};");

	mihl_add(cnx, "function showData2( originalRequest ) {");
	mihl_add(cnx,
		 "  document.getElementById('field2').childNodes[0].nodeValue = originalRequest.responseText;");
//  mihl_add( cnx, "  field2.innerText = originalRequest.responseText;" );
	mihl_add(cnx, "};");

	mihl_add(cnx, "function showData3( originalRequest ) {");
	mihl_add(cnx,
		 "  document.getElementById('field3').childNodes[0].nodeValue = originalRequest.responseText;");
