 ...
 */
static int add_new_connexion(mihl_ctx_t * ctx, int sockfd,
			     struct sockaddr_in *client_addr)
{
 ...
 oops:		;
		mihl_log(ctx, MIHL_LOG_INFO,
			 "Too many connexions (%d): connexion refused",
			 ctx->nb_connexions);
	...
	cnx->active = 1;
	cnx->sockfd = sockfd;
	memmove(&cnx->info.client_addr, client_addr,
	...
	mihl_log(ctx, MIHL_LOG_INFO_VERBOSE,
...
static void delete_connexion(mihl_cnx_t * cnx)
{
	...
	mihl_log(ctx, MIHL_LOG_INFO_VERBOSE, "Delete connexion for socket %d",
		 cnx->sockfd);
...
static int bind_and_listen(mihl_ctx_t * ctx)
{
	...
	if (setsockopt
	    (ctx->sockfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&flag,
	     sizeof(flag)) < 0) {
		mihl_log(ctx, MIHL_LOG_ERROR, "Unable to setsockopt a socket!");
		exit(-1);
	}
	// Non blocking Socket
	fcntl(ctx->sockfd, F_SETFL,
	      fcntl(ctx->sockfd, F_GETFL, 0) | O_NONBLOCK);
	...
	int status =
	    bind(ctx->sockfd, (struct sockaddr *)&server_addr,
		 sizeof(struct sockaddr_in));
	if ((status == SOCKET_ERROR) && (ERRNO == EADDRINUSE)) {
		mihl_log(ctx, MIHL_LOG_ERROR,
			 "Unable to bind, port %d is already in use",
			 ctx->port);
		if (close(ctx->sockfd) == -1)
			mihl_log(ctx, MIHL_LOG_ERROR,
				 "Error %d while closing socket %d", errno,
				 ctx->sockfd);
		return -1;
	}
	if (status < 0) {
		mihl_log(ctx, MIHL_LOG_ERROR,
			 ...
			 ctx->port, status, strerror(errno));
		if (close(ctx->sockfd) == -1)
			mihl_log(ctx, MIHL_LOG_ERROR,
				 "Error %d while closing socket %d", errno,
				 ctx->sockfd);
 ...
 * @return TBD
 */
static int page_not_found(mihl_cnx_t * cnx, char const *tag, char const *host,
			  void *param)
{
	mihl_add(cnx, "<html>");
	mihl_add(cnx, "<head>");
 ...
 */
mihl_ctx_t *mihl_init(char const *bind_addr, int port, int maxnb_cnx,
		      unsigned log_level)
{
	int ncnx;
	...
	ctx->connexions =
	    (mihl_cnx_t *) malloc(sizeof(mihl_cnx_t) * ctx->maxnb_cnx);
	      ...
	      char *content_type, int close_connection)
{
	...
	if (dcount == -1) {
		mihl_log(cnx->ctx, MIHL_LOG_ERROR, "*** %s %d: OOPS - %m!!!!!",
			 __FILE__, __LINE__);
		return -1;
	}
	...
	for (;;) {
		...
		if (dcount == -1) {
			mihl_log(cnx->ctx, MIHL_LOG_ERROR,
				 "*** %s %d: - %m!!!!!", __FILE__, __LINE__);
			return -1;
		}
		rem -= count;
		if (rem == 0)
 ...
 * @return TBD
 */
static int search_for_handle(mihl_cnx_t * cnx, char const *tag, char *host,
			     int nb_variables, char **vars_names,
			     char **vars_values)
{
	int n;
	mihl_ctx_t *ctx = cnx->ctx;
	...
	for (n = 0; n < ctx->nb_handles; n++) {
		...
		if (handle->tag &&
		    ((!handle->partial && !strcmp(tag, handle->tag))
		     || (handle->partial
			 && !strncmp(tag, handle->tag, handle->partial)))) {
			if (handle->pf_get)
				return handle->pf_get(cnx, tag, host,
						      handle->param);
			if (handle->pf_post)
				return handle->pf_post(cnx, tag, host,
						       nb_variables, vars_names,
						       vars_values,
						       handle->param);
			return send_file(cnx, tag, handle->filename,
					 handle->content_type,
					 handle->close_connection);
		}
	}
	if (handle_nfound)
		return handle_nfound->pf_get(cnx, tag, host,
					     handle_nfound->param);
	return 0;
}				// search_for_handle
	...
	for (;;) {
		...
		if (sockfd_accept == -1) {
			...
			if (errno == EINTR) {	// A signal has been applied
				mihl_log(ctx, MIHL_LOG_ERROR, "!!! %d !!!",
					 __LINE__);
//              Sleep( 500 );
				continue;
			}
			mihl_log(ctx, MIHL_LOG_ERROR,
				 "Socket non accepted - errno=%d", ERRNO);
			if (close(ctx->sockfd) == -1) {
				mihl_log(ctx, MIHL_LOG_ERROR,
					 "Error %d while closing socket %d",
					 errno, ctx->sockfd);
			}
			exit(-1);
		}		// if
...
static int got_data_for_active_connexion(mihl_cnx_t * cnx)
{
	...
	for (;;) {
		int l =
		    tcp_read(cnx->sockfd, &ctx->read_buffer[len],
			     ctx->read_buffer_maxlen - len);
		if (l == 0)
			break;
		len += l;
	...
	if (ctx->log_level & MIHL_LOG_DEBUG) {
		mihl_log(cnx->ctx, MIHL_LOG_DEBUG, "%d:[%s]", cnx->sockfd,
			 ctx->read_buffer);
	} else {
		...
		if (p)
			*p = 0;
		mihl_log(cnx->ctx, MIHL_LOG_INFO_VERBOSE, "[%s]",
			 ctx->read_buffer);
		if (p)
			*p = '\015';
	}
...
#if 0
	{
		...
		printf("%3d: len=%d / %d  nbopt=%d nbvars=%d\015\12", cpt, len,
		       ctx->read_buffer_maxlen, nb_options, nb_variables);
		fflush(stdout);
		char fname[100];
		...
		fprintf(fp, "%3d: len=%d / %d  nbopt=%d nbvars=%d\015\12",
			cpt, len, ctx->read_buffer_maxlen, nb_options,
			nb_variables);
		fprintf(fp, "\015\012");
		for (int n = 0; n < nb_options; n++)
			fprintf(fp, "  %2d: %s = [%s]\015\012", n,
				options_names[n], options_values[n]);
		fprintf(fp, "\015\012");
		for (int n = 0; n < nb_variables; n++)
			fprintf(fp, "  %2d: %s = [%s]\015\012", n,
				vars_names[n], vars_values[n]);
		fclose(fp);
	}
#endif
...
static int manage_timedout_connexions(mihl_ctx_t * ctx, time_t now)
{
	...
	for (ncnx = 0; ncnx < ctx->maxnb_cnx; ncnx++) {
		...
		if (t >= cnx->keep_alive) {
			mihl_log(cnx->ctx, MIHL_LOG_DEBUG,
				 "*** DELETE CNX %d - %ld %ld -> %d", ncnx, now,
				 cnx->info.time_last_data, t);
			       ...
			       mihl_pf_handle_get_t * pf, void *param)
{
	if (ctx->handles == NULL)
		...
		ctx->handles =
		    (mihl_handle_t *) realloc(ctx->handles,
					      sizeof(mihl_handle_t) *
					      (ctx->nb_handles + 1));
	if (!ctx->handles)
		return -1;
...
int mihl_dump_info(mihl_ctx_t * ctx)
{
	...
	mihl_log(ctx, MIHL_LOG_DEBUG, "%d active connexions",
		 ctx->nb_connexions);
	if (ctx->nb_connexions == 0)
		return 0;
	mihl_log(ctx, MIHL_LOG_DEBUG,
		 "Sockfd Client               Start Inact Last Request");
	time_t now = time(NULL);
	...
	for (ncnx = 0; ncnx < ctx->maxnb_cnx; ncnx++) {
		...
		if (cnx->active) {
			...
			strncpy(client,
				inet_ntoa(cnx->info.client_addr.sin_addr), 20);
			client[20] = 0;
			mihl_log(ctx, MIHL_LOG_DEBUG,
				 "%6d %-20s %4d\" %4d\" %s", cnx->sockfd,
				 client, (int)(now - cnx->info.time_started),
...
int mihl_info(mihl_ctx_t * ctx, int maxnb_cnxinfos, mihl_cnxinfo_t * infos)
{
	...
	for (ncnx = 0; ncnx < ctx->maxnb_cnx; ncnx++) {
		...
		if (!cnx->active)
			continue;
		memmove(&infos[nb_cnxinfos++], &cnx->info,
			sizeof(mihl_cnxinfo_t));
