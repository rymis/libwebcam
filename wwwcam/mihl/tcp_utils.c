...
int tcp_write(int sockfd, const char *buff, int buff_len)
{
	...
	for (;;) {
		...
		int dcount =
		    send(sockfd, (const char *)pbuff, len, MSG_NOSIGNAL);

		if ((dcount == -1) && (errno == EPIPE)) {
			printf("%s %d : >>> %d <<<<\015\012", __FILE__,
			       __LINE__, ERRNO);
			fflush(stdout);
			return -1;
		}
...
void decode_keys_values(mihl_cnx_t * cnx, char *_request,
			int *nb_options, char *options_names[],
			char *options_values[], int maxnb_options,
			int *nb_variables, char *vars_names[],
			char *vars_values[], int maxnb_values)
{
	int n;
...
int mihl_add(mihl_cnx_t * cnx, char const *fmt, ...)
{
	if (cnx->html_buffer_len + 1024 >= cnx->html_buffer_sz) {
		...
		cnx->html_buffer =
		    (char *)realloc(cnx->html_buffer, cnx->html_buffer_sz);
	}
	...
	va_start(ap, fmt);
	int len =
	    vsnprintf(&cnx->html_buffer[cnx->html_buffer_len], 1024 - 3, fmt,
		      ap);
	va_end(ap);
...
int mihl_send(mihl_cnx_t * cnx, char const *answer, char const *fmt_header, ...)
{
	...
	int len1 =
	    vsnprintf(&header[l], sizeof(header) - l - 1, fmt_header, ap);
	va_end(ap);
	...
	if (count == -1) {
		printf("\n>>> %s %d: OOPS %d!!!!!\015\012", __FILE__, __LINE__,
		       ERRNO);
		fflush(stdout);
		return -1;
	}
	...
	if (count != -1) {
		count =
		    tcp_write(cnx->sockfd, cnx->html_buffer,
			      cnx->html_buffer_len);
		if (count == -1) {
			printf("\n>>> %s %d: OOPS %d!!!!!\n", __FILE__,
			       __LINE__, ERRNO);
